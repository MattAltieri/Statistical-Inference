...
dat <- data.frame(
    x=c(apply(matrix(sample(0:1, nosim * 10, replace=T),
                     nosim), 1, cfunc, 10),
        apply(matrix(sample(0:1, nosim * 20, replace=T),
                     nosim), 1, cfunc, 20),
        apply(matrix(sample(0:1, nosim * 30, replace=T),
                     nosim), 1, cfunc, 30)
        ),
    size=factor(rep(c(10, 20, 30), rep(nosim, 3))))
g <- ggplot(dat, aes(x=x, fill=size)) +
    geom_histogram(binwidth=.3,  color="black", aes(y=..density..)) +
    stat_function(fun=dnorm, size=2) +
    facet_grid(. ~ size)
g
```
...
dat <- data.frame(
    x=c(apply(matrix(sample(0:1, prob=c(.1, .9), nosim * 10, replace=T),
                     nosim), 1, cfunc, 10),
        apply(matrix(sample(0:1, prob=c(.1, .9), nosim * 20, replace=T),
                     nosim), 1, cfunc, 20),
        apply(matrix(sample(0:1, prob=c(.1, .9), nosim * 30, replace=T),
                     nosim), 1, cfunc, 30)),
    size=factor(rep(c(10, 20, 30), rep(nosim, 3))))
g <- ggplot(dat, aes(x=x, fill=size)) +
    geom_histogram(binwidth=.3, color="black", aes(y=..density..)) +
...
```{r, echo=FALSE, fig.align='center', fig.height=6, fig.width=6}
ggplot(data.frame(pvals, coverage2), aes(x=pvals, y=coverage2)) +
    geom_line(size=2) +
    geom_hline(yintercept=0.95) +
    ylim(0.75, 1.0)
```
